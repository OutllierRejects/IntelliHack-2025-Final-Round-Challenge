...
export const authService = {
  login: async (credentials: LoginForm): Promise<{ user: AuthUser; token: string }> => {
    return api.post(endpoints.auth.login, credentials)
  },

  register: async (userData: RegisterForm): Promise<{ user: AuthUser; token: string }> => {
...
export const requestService = {
  getRequests: async (filters?: FilterOptions): Promise<PaginatedResponse<HelpRequest>> => {
  ...
  updateRequest: async (id: string, updates: Partial<HelpRequest>): Promise<HelpRequest> => {
...
export const taskService = {
  getTasks: async (filters?: FilterOptions): Promise<PaginatedResponse<Task>> => {
...
export const resourceService = {
  getResources: async (filters?: FilterOptions): Promise<PaginatedResponse<Resource>> => {
...
export const userService = {
  ...
  updateAvailability: async (available: boolean): Promise<User> => {
    return api.patch(endpoints.users.profile, { availability_status: available })
  },
}
