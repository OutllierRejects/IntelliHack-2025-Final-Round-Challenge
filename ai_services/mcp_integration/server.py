...
class DisasterResponseMCPServer:
                ...
                Tool(
                    ...
                    inputSchema={
                        ...
                        "properties": {
                            "resource_type": {
                                "type": "string",
                                "enum": ["FOOD", "WATER", "MEDICAL", "SHELTER", "TRANSPORT", "EQUIPMENT"],
    ...
    async def _process_emergency_request(self, args: Dict[str, Any]) -> List[TextContent]:
        """Process emergency request through agent pipeline"""
        try:
            if not self.agent_coordinator:
                return [TextContent(type="text", text="Agent coordinator not available")]
            ...
            processing_result = await self.agent_coordinator._process_single_request(request_data)
    ...
    async def _assign_volunteer_to_task(self, args: Dict[str, Any]) -> List[TextContent]:
    ...
    async def _get_available_resources(self, args: Dict[str, Any]) -> List[TextContent]:
        ...
        try:
            query = self.supabase.table('resources').select('*').eq('status', 'AVAILABLE')
    ...
    async def _prioritize_requests(self, args: Dict[str, Any]) -> List[TextContent]:
        ...
        try:
            ...
            if request_ids:
                ...
                result = await self.agent_coordinator.process_incidents_batch(request_ids)
            else:
    ...
    async def _get_active_requests_resource(self) -> str:
        ...
        try:
            result = self.supabase.table('requests').select('*').neq('status', 'COMPLETED').execute()
    ...
    async def _get_agent_status_resource(self) -> str:
        ...
        try:
            if not self.agent_coordinator:
                return json.dumps({"status": "unavailable", "message": "Agent coordinator not initialized"})
