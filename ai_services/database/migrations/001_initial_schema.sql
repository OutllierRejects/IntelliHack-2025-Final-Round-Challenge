...
CREATE TABLE public.user_profiles (
  ...
  skills TEXT [],
  -- Array of skills for volunteers/responders
...
CREATE TABLE public.requests (
  ...
  location VARCHAR(255),
  needs TEXT [] DEFAULT '{}',
  -- Array of needs (food, water, medical, etc.)
...
CREATE TABLE public.tasks (
  ...
  -- JSON object of resource requirements
...
CREATE TABLE public.communications (
  ...
  message TEXT NOT NULL,
  communication_type VARCHAR(50) DEFAULT 'message',
  -- message, status_update, assignment, completion
...
CREATE TRIGGER update_requests_updated_at BEFORE
UPDATE
  ON public.requests FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_tasks_updated_at BEFORE
UPDATE
  ON public.tasks FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_resources_updated_at BEFORE
UPDATE
  ON public.resources FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
ALTER TABLE
  public.requests ENABLE ROW LEVEL SECURITY;

ALTER TABLE
  public.tasks ENABLE ROW LEVEL SECURITY;

ALTER TABLE
  public.resources ENABLE ROW LEVEL SECURITY;

ALTER TABLE
  public.notifications ENABLE ROW LEVEL SECURITY;

ALTER TABLE
  public.communications ENABLE ROW LEVEL SECURITY;
...
SELECT
  USING (
    ...
    OR EXISTS (
      ...
      WHERE
        id = auth.uid()
        AND role IN ('first_responder', 'admin')
    )
  );
...
CREATE POLICY "Users can update own requests" ON public.requests FOR
UPDATE
  USING (
    auth.uid() = user_id
    OR EXISTS (
...
CREATE POLICY "Task visibility" ON public.tasks FOR
SELECT
  USING (
...
CREATE POLICY "Resource visibility" ON public.resources FOR
SELECT
  USING (TRUE);
...
CREATE POLICY "Communication access" ON public.communications FOR
SELECT
  USING (
