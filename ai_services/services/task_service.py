    ...
    try:
        ...
        if filters:
            for key, value in filters.items():
                ...
                elif key in ["status", "priority", "task_type", "assignee_id", "request_id"]:
                    query = query.eq(key, value)
...
def assign_task(task_id: str, assignee_id: str, assigned_by: str = None) -> Optional[Dict]:
...
def get_available_tasks(user_role: str = None, location: str = None) -> List[Dict]:
    ...
    try:
        ...
        response = query.order("priority", desc=True).order("created_at", desc=False).execute()
