# ai_services/services/resource_service.py
...
def update_resource_stock(resource_id: str, change_amount: int, operation: str = "decrement") -> Optional[Dict]:
    ...
    try:
        ...
        if response.data:
            logger.info(f"Updated resource {resource_id} stock: {current_stock} -> {new_stock}")
            
            # Check if below threshold
            threshold = current_resource.get("threshold", 0)
            if new_stock <= threshold:
                logger.warning(f"Resource {resource_id} is below threshold: {new_stock} <= {threshold}")
