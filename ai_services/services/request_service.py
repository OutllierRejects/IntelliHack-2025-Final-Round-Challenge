...
def create_request(request_data: Dict) -> Dict:
    ...
    try:
        ...
        request_data.update({"created_at": now, "updated_at": now, "status": "pending"})
...
def get_dashboard_stats() -> Dict:
    ...
    try:
        ...
        for status in ["pending", "processing", "assigned", "completed"]:
            response = (
                supabase.table("requests").select("id").eq("status", status).execute()
            )
        ...
        for priority in ["critical", "high", "medium", "low"]:
            ...
            stats[f"{priority}_priority_count"] = (
